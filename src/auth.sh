...
function azLogin() {
    ...
    case $auth_method in
        ...
        interactive)
            ...
            ;;
        device-code)
            echo "Logging in using device code..."
            if az login --use-device-code > /dev/null 2>&1; then
                echo "Successfully logged in to Azure."
            else
                echo "Failed to login to Azure using device code. Please try again."
                exit 1
            fi
            ;;
        browser)
            echo "Logging in using browser..."
            if az login > /dev/null 2>&1; then
                echo "Successfully logged in to Azure."
            else
                echo "Failed to login to Azure using browser. Please try again."
                exit 1
            fi
            ;;
        *)
            echo "Unknown authentication method: $auth_method"
            echo "Please use one of the following methods: service-principal, interactive, device-code, browser"
            exit 1
            ;;
    esac
}
...
function getSubscriptions() {
    ...
    read -p "Enter the subscription numbers to scan (comma-separated) or press Enter to scan all: " selected_subs

    local selected_sub_ids=()
    local selected_sub_names=()

    if [[ -z "$selected_subs" ]]; then
        # User wants to scan all subscriptions
        selected_sub_ids=($(echo "$subscriptions" | awk '{print $1}'))
        selected_sub_names=($(echo "$subscriptions" | awk '{print $2}'))
    else
