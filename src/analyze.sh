...
function analyzeData() {
 ...
 log "Analyzing resource: $resource_name ($resource_type)" "INFO"
 ...
 *)
 ...
 log "Unknown resource type: $resource_type" "WARNING"
 ;;
 esac
...
function analyzeApiManagementService() {
 ...
 log "API Management service $resource_name is not using the Premium SKU" "WARNING"
 fi
 ...
 log "API Management service $resource_name has public network access enabled" "WARNING"
 fi
 ...
 log "API Management service $resource_name is not connected to a virtual network" "WARNING"
 fi
}
function analyzeVmExtension() {
 ...
 log "VM extension $resource_name has a provisioning state of $extension_provisioning_state" "WARNING"
 fi
 case $extension_type in
 "AzureDiskEncryption")
 # Check if Azure Disk Encryption extension is not enabled
 for ((retry=1; retry<=MAX_RETRIES; retry++)); do
 encryption_operation=$(timeout $TIMEOUT_DURATION az vm encryption show --name "${resource_name%%/*}" --resource-group "${resource_id##*/resourceGroups/}" --query "encryptionOperation" -o tsv 2>/dev/null)
 if [[ $? -eq 0 ]]; then
 if [[ "$encryption_operation" != "EnableEncryption" ]]; then
 FINDINGS+=("Azure Disk Encryption extension $resource_name is not enabled")
 log "Azure Disk Encryption extension $resource_name is not enabled" "WARNING"
 fi
 break
 else
 echo "Error checking Azure Disk Encryption extension status for $resource_name. Retry $retry/$MAX_RETRIES."
 log "Error checking Azure Disk Encryption extension status for $resource_name. Retry $retry/$MAX_RETRIES." "ERROR"
 sleep $((retry * 2))
 fi
 done
 ;;
 ...
 for ((retry=1; retry<=MAX_RETRIES; retry++)); do
 workspace_id=$(timeout $TIMEOUT_DURATION az vm extension show --name "$resource_name" --resource-group "${resource_id##*/resourceGroups/}" --query "settings.workspaceId" -o tsv 2>/dev/null)
 if [[ $? -eq 0 ]]; then
 if [[ -z "$workspace_id" ]]; then
 FINDINGS+=("Microsoft Monitoring Agent extension $resource_name is not connected to a Log Analytics workspace")
 log "Microsoft Monitoring Agent extension $resource_name is not connected to a Log Analytics workspace" "WARNING"
 fi
 break
 else
 echo "Error checking Microsoft Monitoring Agent extension configuration for $resource_name. Retry $retry/$MAX_RETRIES."
 log "Error checking Microsoft Monitoring Agent extension configuration for $resource_name. Retry $retry/$MAX_RETRIES." "ERROR"
 sleep $((retry * 2))
 fi
 done
 ;;
 ...
 for ((retry=1; retry<=MAX_RETRIES; retry++)); do
 enable_ama=$(timeout $TIMEOUT_DURATION az vm extension show --name "$resource_name" --resource-group "${resource_id##*/resourceGroups/}" --query "settings.enableAMA" -o tsv 2>/dev/null)
 if [[ $? -eq 0 ]]; then
 if [[ "$enable_ama" != "true" ]]; then
 FINDINGS+=("Dependency Agent extension $resource_name is not enabled for Azure Monitor for VMs")
 log "Dependency Agent extension $resource_name is not enabled for Azure Monitor for VMs" "WARNING"
 fi
 break
 else
 echo "Error checking Dependency Agent extension configuration for $resource_name. Retry $retry/$MAX_RETRIES."
 log "Error checking Dependency Agent extension configuration for $resource_name. Retry $retry/$MAX_RETRIES." "ERROR"
 sleep $((retry * 2))
 fi
 done
 ;;
 ...
 for ((retry=1; retry<=MAX_RETRIES; retry++)); do
 antimalware_enabled=$(timeout $TIMEOUT_DURATION az vm extension show --name "$resource_name" --resource-group "${resource_id##*/resourceGroups/}" --query "settings.AntimalwareEnabled" -o tsv 2>/dev/null)
 if [[ $? -eq 0 ]]; then
 if [[ "$antimalware_enabled" != "true" ]]; then
 FINDINGS+=("Antimalware extension $resource_name is not enabled")
 log "Antimalware extension $resource_name is not enabled" "WARNING"
 fi
 break
 else
 echo "Error checking Antimalware extension configuration for $resource_name. Retry $retry/$MAX_RETRIES."
 log "Error checking Antimalware extension configuration for $resource_name. Retry $retry/$MAX_RETRIES." "ERROR"
 sleep $((retry * 2))
 fi
 done
 ;;
 *)
 ...
 log "Unknown VM extension type: $extension_type" "WARNING"
 ;;
 esac
}
function analyzeConnectionMonitor() {
 ...
 log "Connection monitor $resource_name has no endpoints defined" "WARNING"
 fi
 ...
 log "Connection monitor $resource_name has no test groups defined" "WARNING"
 fi
}
function analyzeScheduledQueryRule() {
 ...
 log "Scheduled query rule $resource_name has no query defined" "WARNING"
 fi
 ...
 log "Scheduled query rule $resource_name has a frequency greater than 60 minutes" "WARNING"
 fi
}
function analyzeHybridMachine() {
 ...
 log "Hybrid machine $resource_name has an unknown OS name: $os_name" "WARNING"
 fi
 ...
 log "Hybrid machine $resource_name has a provisioning state of $provisioning_state" "WARNING"
 fi
}
function analyzeCdnWafPolicy() {
 ...
 log "CDN WAF policy $resource_name is not enabled" "WARNING"
 fi
 ...
 log "CDN WAF policy $resource_name is not in Prevention mode" "WARNING"
 fi
}
function analyzeResourceGraphQuery() {
 ...
 log "Resource Graph query $resource_name has no query defined" "WARNING"
 fi
 ...
 log "Resource Graph query $resource_name contains a potentially expensive operation (join or union)" "WARNING"
 fi
}
function analyzeExpressRouteCircuit() {
 ...
 log "Express Route circuit $resource_name is using the Standard SKU (consider upgrading to Premium)" "WARNING"
 fi
 ...
 log "Express Route circuit $resource_name has no peering location defined" "WARNING"
 fi
}
function analyzeVpnGateway() {
 ...
 log "VPN gateway $resource_name has an unexpected gateway type: $gateway_type" "WARNING"
 fi
 ...
 log "VPN gateway $resource_name is not using route-based VPN" "WARNING"
 fi
}
function analyzeVirtualNetworkGateway() {
 ...
 log "Virtual network gateway $resource_name is using the Basic SKU (consider upgrading)" "WARNING"
 fi
 ...
 log "Virtual network gateway $resource_name does not have active-active mode enabled" "WARNING"
 fi
}
function analyzeTemplateSpecVersion() {
 ...
 log "Template spec version $resource_name has no description" "WARNING"
 fi
 ...
 log "Template spec version $resource_name has an unexpected artifact type: $artifact_type" "WARNING"
 fi
}
function analyzeDataLakeStoreAccount() {
 ...
 log "Data Lake Store account $resource_name does not have encryption enabled" "WARNING"
 fi
 ...
 log "Data Lake Store account $resource_name does not have firewall enabled" "WARNING"
 fi
}
function analyzeAnalysisServicesServer() {
 ...
 log "Analysis Services server $resource_name has no backup blob container URI defined" "WARNING"
 fi
 ...
 log "Analysis Services server $resource_name is not using the 'All' query pool connection mode" "WARNING"
 fi
}
function analyzeFrontDoorWafPolicy() {
 ...
 log "Front Door WAF policy $resource_name is not enabled" "WARNING"
 fi
 ...
 log "Front Door WAF policy $resource_name is not in Prevention mode" "WARNING"
 fi
}
function analyzeFirewallPolicy() {
 ...
 log "Firewall policy $resource_name has threat intelligence mode turned off" "WARNING"
 fi
 ...
 log "Firewall policy $resource_name does not have SQL injection detection set to Prevent" "WARNING"
 fi
}
function analyzeLoadBalancer() {
 ...
 log "Load balancer $resource_name is using the Basic SKU (consider upgrading to Standard)" "WARNING"
 fi
 ...
 log "Load balancer $resource_name has no health probes defined" "WARNING"
 fi
}
function analyzeLocalNetworkGateway() {
 ...
 log "Local network gateway $resource_name has no gateway IP address defined" "WARNING"
 fi
}
function analyzeCognitiveServicesAccount() {
 ...
 log "Cognitive Services account $resource_name is using the free tier (F0)" "WARNING"
 fi
 ...
 log "Cognitive Services account $resource_name does not have network ACLs set to deny by default" "WARNING"
 fi
}
function analyzeSearchService() {
 ...
 log "Search service $resource_name has less than 2 partitions" "WARNING"
 fi
 ...
 log "Search service $resource_name has less than 2 replicas" "WARNING"
 fi
}
function analyzeAppGatewayWafPolicy() {
 ...
 log "Application Gateway WAF policy $resource_name has no policy settings defined" "WARNING"
 fi
 ...
 log "Application Gateway WAF policy $resource_name has no managed rules defined" "WARNING"
 fi
}
function analyzeAzureFirewall() {
 ...
 log "Azure Firewall $resource_name has threat intel mode set to $threat_intel_mode (should be Alert or Deny)" "WARNING"
 fi
 ...
 log "Azure Firewall $resource_name is associated with a virtual hub (consider using a hub-spoke topology)" "WARNING"
 fi
}
function analyzeApplicationGateway() {
 ...
 log "Application Gateway $resource_name is not using a v2 SKU" "WARNING"
 fi
 ...
 log "Application Gateway $resource_name does not have WAF enabled" "WARNING"
 fi
}
function analyzePrivateEndpoint() {
 ...
 log "Private Endpoint $resource_name has no private link service connections" "WARNING"
 fi
}
function analyzeContainerRegistry() {
 ...
 log "Container Registry $resource_name has admin user enabled" "WARNING"
 fi
 ...
 log "Container Registry $resource_name does not have network rules set to deny by default" "WARNING"
 fi
}
function analyzeVpnSite() {
 ...
 log "VPN Site $resource_name is not associated with a Virtual WAN" "WARNING"
 fi
 ...
 log "VPN Site $resource_name has no address space defined" "WARNING"
 fi
}
function analyzeManagedCluster() {
 ...
 log "Managed Cluster (AKS) $resource_name is using an outdated Kubernetes version: $kubernetes_version" "WARNING"
 fi
 ...
 log "Managed Cluster (AKS) $resource_name is not using the Azure CNI network plugin" "WARNING"
 fi
}
function analyzeWebAppSlot() {
 ...
 log "Web App Slot $resource_name does not have HTTPS only enabled" "WARNING"
 fi
 ...
 log "Web App Slot $resource_name is not using the minimum TLS version of 1.2" "WARNING"
 fi
}
function analyzeRouteTable() {
 ...
 log "Route Table $resource_name has no routes defined" "WARNING"
 fi
 ...
 log "Route Table $resource_name has a route with 0.0.0.0/0 next hop (potential security risk)" "WARNING"
 fi
}
function analyzeCosmosDBAccount() {
 ...
 log "Cosmos DB account $resource_name has public network access enabled" "WARNING"
 fi
 ...
 log "Cosmos DB account $resource_name has a backup interval greater than 4 hours" "WARNING"
 fi
}
function analyzeWebApp() {
 ...
 log "Web App $resource_name does not have HTTPS only enabled" "WARNING"
 fi
 ...
 log "Web App $resource_name does not have client certificates enabled" "WARNING"
 fi
}
function analyzePublicIpAddress() {
 ...
 log "Public IP address $resource_name is not using static allocation" "WARNING"
 fi
 ...
 log "Public IP address $resource_name has an idle timeout less than 5 minutes" "WARNING"
 fi
}
function analyzeKeyVault() {
 ...
 log "Key Vault $resource_name does not have soft delete enabled" "WARNING"
 fi
 ...
 log "Key Vault $resource_name does not have purge protection enabled" "WARNING"
 fi
}
function analyzeNSG() {
 local resource=$1
 ...
 log "NSG $resource_name has a default security rule allowing inbound access from any source" "WARNING"
 fi
 if [[ "$security_rules" =~ (\"[^\"]*\*[^\"]*\"|\"0\.0\.0\.0\/0\") ]]; then
 FINDINGS+=("NSG $resource_name has a security rule allowing inbound access from any source")
 log "NSG $resource_name has a security rule allowing inbound access from any source" "WARNING"
 fi
}
function analyzeVirtualNetwork() {
 ...
 log "Virtual Network $resource_name has an address space larger than /8: $address_space" "WARNING"
 fi
 # Check if virtual network has no subnets defined
 if [[ -z "$subnets" ]]; then
 FINDINGS+=("Virtual Network $resource_name has no subnets defined")
 log "Virtual Network $resource_name has no subnets defined" "WARNING"
 fi
}
function analyzeStorageAccount() {
 ...
 log "Storage Account $resource_name does not have HTTPS traffic only enabled" "WARNING"
 fi
 ...
 log "Storage Account $resource_name does not have network ACLs set to deny by default" "WARNING"
 fi
}
function analyzeNetworkInterface() {
 ...
 log "Network Interface $resource_name has IP forwarding enabled" "WARNING"
 fi
 ...
 log "Network Interface $resource_name does not have accelerated networking enabled" "WARNING"
 fi
}
function analyzeNatGateway() {
 ...
 log "NAT Gateway $resource_name has no public IP addresses defined" "WARNING"
 fi
...
if [[ "$idle_timeout" -lt 4 ]]; then
 ...
 log "NAT Gateway $resource_name has an idle timeout less than 4 minutes" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"resourceGroup":\s*"\K[^"]+"') ]]; then
 ...
 log "Network Intent Policy $resource_name has no resource group defined" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"addressPrefix":\s*"\K[^"]+"') ]]; then
 ...
 log "Virtual Hub $resource_name has no address prefix defined" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"provisioningState":\s*"\K[^"]+"') != "Succeeded" ]]; then
 ...
 log "Application Security Group $resource_name provisioning state is not 'Succeeded'" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"dnsName":\s*"\K[^"]+"') ]]; then
 ...
 log "Bastion Host $resource_name has no DNS name defined" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"virtualNetworks":\s*\K[^,}]+' | grep -o '{' | wc -l) -eq 0 ]]; then
 ...
 log "DDOS Protection Plan $resource_name has no associated virtual networks" "WARNING"
fi
}
...
if [[ "$connection_type" != "IPsec" && "$connection_type" != "ExpressRoute" ]]; then
 ...
 log "Connection $resource_name has an invalid connection type: $connection_type" "WARNING"
fi
...
if [[ -z $(echo "$resource" | grep -oP '"virtualNetworkGateway1":\s*{\s*"id":\s*"\K[^"]+"') || -z $(echo "$resource" | grep -oP '"virtualNetworkGateway2":\s*{\s*"id":\s*"\K[^"]+"') ]]; then
 ...
 log "Connection $resource_name is missing one or both virtual network gateways" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"domainJoined":\s*\K[^,}]+') != "true" ]]; then
 ...
 log "DevCenter Network Connection $resource_name is not domain joined" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"linkedResolver":\s*{\s*"id":\s*"\K[^"]+"') ]]; then
 ...
 log "DNS Resolver Outbound Endpoint $resource_name has no linked resolver" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"dnsResolverOutboundEndpoints":\s*\K[^,}]+' | grep -o '{' | wc -l) -eq 0 ]]; then
 ...
 log "DNS Forwarding Ruleset $resource_name has no outbound endpoints defined" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"allowVnetToVnetTraffic":\s*\K[^,}]+') != "true" ]]; then
 ...
 log "Virtual WAN $resource_name does not allow VNet-to-VNet traffic" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"serviceEndpointPolicyDefinitions":\s*\K[^,}]+' | grep -o '{' | wc -l) -eq 0 ]]; then
 ...
 log "Service Endpoint Policy $resource_name has no policy definitions" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"virtualHub":\s*{\s*"id":\s*"\K[^"]+"') ]]; then
 ...
 log "Express Route Gateway $resource_name is not associated with a virtual hub" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"virtualNetwork":\s*{\s*"id":\s*"\K[^"]+"') ]]; then
 ...
 log "DNS Resolver $resource_name has no virtual network ID" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"ipConfigurations":\s*\K[^,}]+' | grep -o '{' | wc -l) -eq 0 ]]; then
 ...
 log "DNS Resolver Inbound Endpoint $resource_name has no IP configurations" "WARNING"
fi
}
...
if [[ -z $(echo "$resource" | grep -oP '"ipPrefix":\s*"\K[^"]+"') ]]; then
 ...
 log "Public IP Prefix $resource_name has no IP prefix defined" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"ipAddresses":\s*\K[^,}]+' | grep -o '[^[:space:]]' | wc -l) -eq 0 ]]; then
 ...
 log "IP Group $resource_name has no IP addresses defined" "WARNING"
fi
}
...
if [[ $(echo "$resource" | grep -oP '"networkManagerScopeAccesses":\s*\K[^,}]+' | grep -o '{' | wc -l) -eq 0 ]]; then
 ...
 log "Network Manager $resource_name has no scope accesses defined" "WARNING"
fi
}
# Run analysis
analyzeData "$@"